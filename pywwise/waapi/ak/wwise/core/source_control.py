...
from pywwise.enums import (
    EReturnOptions,
    ESourceControlSearchFilter,
    ESourceFileReturnOptions,
)
...
class SourceControl:
    ...
    def add(self, files: ListOrTuple[SystemPath]) -> Union[tuple[LogItem, ...], bool]:
        """
    ...
    def check_out(
        self, files: ListOrTuple[SystemPath]
    ) -> Union[tuple[LogItem, ...], bool]:
        """
        ...
        result = self._client.call(
            "ak.wwise.core.sourceControl.checkOut", {"files": files}
        )
    ...
    def commit(
        self, files: ListOrTuple[SystemPath], message: str
    ) -> Union[tuple[LogItem, ...], bool]:
        """
        ...
        files = list(dict.fromkeys(files))  # convert to list of unique items
        result = self._client.call(
            "ak.wwise.core.sourceControl.commit", {"files": files, "message": message}
        )
    ...
    def get_source_files(
        ...
        results = self._client.call(
            "ak.wwise.core.sourceControl.getSourceFiles", args, options=options
        )
        if results is None:
            return ()
        ...
        for result in results.get("return", ()):
            ...
            usage = tuple(
                [WwiseObjectInfo.from_dict(obj) for obj in result.get("usage", ())]
            )
    ...
    def get_status(
        self, files: ListOrTuple[tuple[SystemPath, SystemPath]]
    ) -> tuple[tuple[LogItem, ...], tuple[SourceControlStatus, ...]]:
        """
        ...
        statuses = tuple(
            [
                SourceControlStatus(
                    result.get("result", dict()).get("status", ""),
    ...
    def revert(
        ...
        files = list(dict.fromkeys(files))  # convert to list of unique items
        result = self._client.call(
            "ak.wwise.core.sourceControl.revert", {"files": files}
        )
